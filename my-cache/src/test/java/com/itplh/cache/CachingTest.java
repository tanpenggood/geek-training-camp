...
import com.itplh.cache.event.TestCacheEntryListener;
...
import static com.itplh.cache.configuration.ConfigurationUtils.cacheEntryListenerConfiguration;
...
public class CachingTest {
    ...
    public void testSampleLettuce() {
        CachingProvider cachingProvider = Caching.getCachingProvider();
        CacheManager cacheManager = cachingProvider.getCacheManager(URI.create("lettuce-redis://127.0.0.1:6379/"), null);
        // configure the cache
        MutableConfiguration<String, Integer> config =
                new MutableConfiguration<String, Integer>()
                        .setTypes(String.class, Integer.class);
        ...
        // create the cache
        Cache<String, Integer> cache = cacheManager.createCache("lettuceRedisCache", config);

        // add listener
        cache.registerCacheEntryListener(cacheEntryListenerConfiguration(new TestCacheEntryListener<>()));

        // cache operations
        String key = "lettuce-redis-key";
        Integer value1 = 1;
        cache.put(key, value1);
